(function() {
  ...
  function processHTML() {
    if (parser.children.length != 1) throw 'HTML should contain 1 root element';

    rootVar = parser.children[0].getAttribute('jsvar');

    processNode(parser.children[0]);

    var parametersString = parameters.filter(onlyUnique).toString().replace(/,/g, ', ');
    result += 'function createDOMElement(' + parametersString + ') {\n' + declarations + appendings + '\n  return ' + rootVar + ';\n}\n';
  }
  ...
  function processNode(node) {
    ...
    if (node.nodeType == 1) {
      if (!node.getAttribute('jsvar')) throw 'jsvar attribute is not set at:\n' + node.outerHTML + '\n';

      var varName = node.getAttribute('jsvar');
      var attributes = node.attributes;

      if (!varRegEx.test(varName)) throw 'Invalid jsvar attribute value: ' + varName;
      if (varList.indexOf(varName) > -1) throw 'Not unique jsvar attribute value: ' + varName;
  ...
  function validateParameters(string) {
    ...
    for (var i = 0; i < list.length; i++) {
      var param = list[i].substr(2, list[i].length - 3);
      if (!varRegEx.test(param)) throw 'Invalid parameter name: ' + param;
  ...
  function processNodeAttribute(attribute, varName) {
    ...
    validateParameters(attribute.value);
    var attrValue  = attribute.value.replace(/\${([^$]+)}/g, '\' + $1 + \'');

    var attrCode = '  ' + varName + '.setAttribute(\'' + attribute.name + '\', \'' + attrValue + '\');\n';
    attrCode = attrCode.replace(/\+ '' \+/g, '+');
    attrCode = attrCode.replace(', \'\' + ', ', ');
    attrCode = attrCode.replace(' + \'\')', ')');

    declarations += attrCode;
  }
